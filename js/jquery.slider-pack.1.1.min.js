...
if(self.settings.autoOpenDescription){if(self.sliderWidth>900){self._showAlbumInfo();self._fadeOut(self._albumIndicator);self.albumsArr[self.currAlbumId].data('sawInfo',true);}}},getAlbumIdByIdAtt:function(idAtt){var self=this,albums=self.albumsArr;for(var i=0;i<albums.length;i++){if(albums[i].attr('data-album-id')===idAtt){return i;}}
...
imgId=this._getImgId(imgName);return imgId;},_blockHashChange:function(){$(window).unbind('hashchange.tds');},_updateAlbumHash:function(){var self=this;self._updateHash(self.albumsArr[self.currAlbumId].attr('data-album-id'));},_unblockHashChange:function(){var self=this;$(window).bind('hashchange.tds',function(){self._onHashChange();});},_updateHash:function(hash){var self=this;self._blockHashChange();window.location.hash=hash;if(self._hashTimeout){clearTimeout(self._hashTimeout);}
self._hashTimeout=setTimeout(function(){self._unblockHashChange();},60);},_onHashChange:function(){var hash=location.hash;var startAlbum=0;var startImgId;var self=this;if(hash){startImgId=this._getImageIdFromHash(hash);startAlbum=this._getAlbumIdFromHash(hash);}
if(startAlbum!==self.currAlbumId){self.init();}else{if(startImgId===self.currItemId){return;}
if(startImgId>self.currItemId){if(startImgId-self.currItemId===1){self._moveTo('next','x',this.settings.transitionSpeed,false,true);}else{self.init();}}else{if(startImgId-self.currItemId===-1){self._moveTo('prev','x',this.settings.transitionSpeed,false,true);}else{self.init();}}}},_getItemLinkText:function(item){var imgLink=$(item).find('a');var imgLinkText=$(imgLink).text();return imgLinkText;},_getImageLinkTextById:function(items,id){var imgNode=items[id];var imgLinkText=this._getItemLinkText(imgNode);return imgLinkText;},_updateImageHashFrag:function(imageId){var album=this.albumsArr[this.currAlbumId];if(album){var items=album.find('.two-dim-item');var imgLinkText=this._getImageLinkTextById(items,imageId);var albumId=$(album).data('albumId');var newHash=albumId+'/'+imgLinkText;this._blockHashChange();this._updateHash(newHash);this._unblockHashChange();}},_getImgId:function(imgTitle){var imgId=0,i,item,imgTitle;var album=this.albumsArr[this.currAlbumId];if(!imgTitle){return 0;}
